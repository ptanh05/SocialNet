          ...
          {showResults && results.length > 0 && (
            <div className="absolute top-full mt-1 left-0 right-0 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-xl shadow-lg overflow-hidden z-50">
              {results.map(u => (
                <button
                  key={u.id}
                  onClick={() => { navigate(`/profile/${u.id}`); setQuery(''); setShowResults(false) }}
                ...
                >
                  ...
                  <span className="text-sm font-medium text-gray-900 dark:text-dark-text truncate">{u.username}</span>
                </button>
              ))}
            </div>
          )}
