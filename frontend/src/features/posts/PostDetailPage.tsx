...
export default function PostDetailPage() {
    ...
    useEffect(() => { if (post) setLikeCount(post.likes_count); }, [post]);
    ...
    const commentMutation = useMutation({
        mutationFn: (data: { content: string; parentId?: number }) => {
            ...
            if (!result.success) {
                const msg = result.error.issues[0]?.message || 'Nội dung bình luận không hợp lệ';
                throw new Error(msg);
            }
            return postsApi.createComment(Number(postId), result.data.content, data.parentId);
        },
        onSuccess: () => {
            setCommentContent('');
            setCommentError('');
    ...
    const likeMutation = useMutation({
        ...
        onMutate: () => { setLiked(!liked); setLikeCount((c) => c + (liked ? -1 : 1)); },
    ...
    const bookmarkMutation = useMutation({
        ...
        onError: () => { setBookmarked(bookmarked); showToast('Không thể lưu bài viết', 'error'); },
        onSettled: () => { queryClient.invalidateQueries({ queryKey: ['bookmarkStatus', postId] }); },
    });

    const handleCommentSubmit = () => {
        const result = commentSchema.safeParse({ content: commentContent });
        if (!result.success) {
            setCommentError(result.error.issues[0]?.message || 'Nội dung không hợp lệ');
            return;
        }
    ...
    const handleReply = (parentId: number, content: string) => commentMutation.mutate({ content, parentId });
    ...
    return (
        <div className='max-w-xl mx-auto space-y-4'>
            <Link to='/feed' className='text-sm text-blue-500 hover:underline dark:text-blue-400'>← Quay lại bảng tin</Link>

            {/* Post */}
            <div className='bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-5 space-y-3'>
                <div className='flex items-center gap-3'>
                    {post.author && (
                        <Link to={`/profile/${post.author.id}`}><Avatar username={post.author.username} /></Link>
                    )}
                    <div>
                        {post.author && (
                ...
                <div className='flex items-center gap-4 pt-3 border-t border-gray-100 dark:border-dark-border'>
                    ...
                    <span className='text-sm text-gray-400 dark:text-dark-muted'>💬 {post.comments_count}</span>
                    <button
            ...
            {user && (
                <div className='bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-4 space-y-3'>
                    <textarea
                        ...
                        className={`w-full border rounded-lg p-3 text-sm resize-none outline-none bg-white dark:bg-dark-bg text-gray-900 dark:text-dark-text placeholder:text-gray-400 focus:ring-2 ${
                            commentError ? 'border-red-500 focus:ring-red-500' : 'border-gray-200 dark:border-dark-border focus:ring-blue-500'
                        }`}
                        rows={3}
                    />
